...
let { error, ok, info, warn } = require("./messages");
...
let { execAsync, extractExercises, testSolution, packet } = _module;
...
const main = () => {
  readLocal("docs/usage.md").then(it => {
    const { help, dir, endpoint, all, justone, num } = getOptions(it);
    if (help) {
      console.log(it);
    } else {
