...
describe('#module', () => {
    ...
    _module = _module({
        debug, _, shelljs, bluebird, agent
    });
    "use strict";
...
describe('#parse', () => {
    ...
    let f = shelljs.cat(`${__dirname}/../fixtures/course.json`);
    f = JSON.parse(f);

    "use strict";
    it('should read fixture', () => {
        return expect(_module.extractExercises(f).length).to.be.equal(14);
    });
    it('should encode a packet', () => {
        let xs = _module.extractExercises(f);
        let packet = _module.packet(xs[0]);
